...
####
####
# 为中心区域：y30kzda8n8.execute-api.eu-central-1.amazonaws.com
请求前缀 /dev/api/v1
## 获取区域信息
[GET /region]
    ...
    示例：
    中心区域：
        https://y30kzda8n8.execute-api.eu-central-1.amazonaws.com/dev/api/v1/region
    + Header 
        Authorization Bearer {accessToken} - 访问令牌, 通过JWT加密得到有时效性的token
        Content-Type: application/json

    + Request （Query参数）
    {
        "registCode": "C29731CC3160A0B2979a",
    }

    + Response 200 (application/json) 
    {
      "url": "2n7d2wilzh.execute-api.ap-east-1.amazonaws.com",
      "host": "a1klotkcn94eo5-ats.iot.ap-east-1.amazonaws.com",
      "region": "ap-east-1"
    }
    url为cloud域名
    host为iot节点
    ...
    {
      "status": 1001,
      "msg": "Get regions faild Cannot read property 'region' of undefined"
    }
## 用户注册 [POST /users]
...
# 中心区域结束
# 为子区域：2n7d2wilzh.execute-api.ap-east-1.amazonaws.com
    请求前缀 /dev/api/v1
